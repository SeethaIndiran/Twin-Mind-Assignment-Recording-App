    ...
    private fun parseSummary(text: String): MeetingSummary? {
        try {
            ...
            for (section in sections) {
                when {
                    section.startsWith("Title:", ignoreCase = true) -> {
                        title = section.substringAfter(":").trim()
                    }
                    section.startsWith("Summary:", ignoreCase = true) -> {
                        summary = section.substringAfter(":").trim()
                    }
                    section.startsWith("Key Points:", ignoreCase = true) -> {
                        keyPoints.addAll(
                    ...
                    section.startsWith("Action Items:", ignoreCase = true) -> {
                        actionItems.addAll(
                            section.substringAfter(":")
                                .trim()
                                .split("\n")
