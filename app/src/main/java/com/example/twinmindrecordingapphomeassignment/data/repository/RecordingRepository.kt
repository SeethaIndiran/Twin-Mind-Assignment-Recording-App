...
class RecordingRepository @Inject constructor(
        ...
        return try {
          ...
          //  Log.d("RecordingRepository", "Chunk sequence: ${chunk.sequence}")
            ...
            val mockTranscriptions = listOf(
                "This is a test recording for the meeting summary application.",
                "Today we discussed the quarterly sales report and marketing strategy.",
                "Action items include updating the website by next Friday.",
                "We need to schedule a follow-up meeting with the design team.",
                "Please review the budget proposal and send feedback by Monday.",
                "The team will prepare the presentation for the client meeting next week."
            )

            val mockText = mockTranscriptions[chunk!!.sequence % mockTranscriptions.size]

         //   Log.d("RecordingRepository", "Mock transcription: $mockText")

            // Update chunk with transcription
           audioChunkDao.updateChunkTranscript(chunkId,true, mockText)

           // Log.d("RecordingRepository", "Chunk updated successfully")*/
    ...
    // Summary operations
    /*suspend fun generateSummary(recordingId: String, transcript: String): Result<MeetingSummary> {
        return try {
            val response = transcriptionService.generateSummary(transcript)
            val summary = response.toMeetingSummary()
    ...
    /*suspend fun generateSummaryStreaming(
        ...
        transcriptionService.generateSummaryStreaming(transcript) { response ->
            val summary = response.toMeetingSummary()
            onUpdate(summary)
    ...
    suspend fun generateSummaryStreaming(
        ...
        chatGptService.generateSummary(transcript) { summary ->
            ...
            // Save final summary to database if it has key points
